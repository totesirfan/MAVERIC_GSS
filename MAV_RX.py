...
class SessionLog:
    ...
    def _maybe_flush(self):
        ...
        if self._writes_since_flush >= self._flush_every:
            self._jsonl_f.flush(); self._text_f.flush()
            self._writes_since_flush = 0
    ...
    def close(self):
        self._jsonl_f.close(); self._text_f.close()
...
def main():
    ...
    try:
        while True:
            ...
            if result is None:
                # Idle spinner with silence timer and rolling rate
            ...
            last_arrival = now; last_watchdog = now
            packet_count += 1
            ...
            is_dup = fp in seen_fps; seen_fps.add(fp)
            ...
            if now - last_render >= RENDER_INTERVAL:
                ...
                last_render = now; render_skipped = 0
            else:
                render_skipped += 1
